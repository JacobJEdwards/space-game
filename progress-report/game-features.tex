...
friction in space.
The player also carries a gun, which can be fired using the left mouse button.
...
switch to the spaceship state, in which the player is now controlling the ship.

The player also has oxygen and health, which are shown in the UI\@.
Oxygen is consumed over time, and regenerates when the player is in the ship.
...
a cooldown time.
The cooldown is shown in the UI\@.
...
The player is able to exit the ship by pressing \textit{F} again, which will cause the player to exit the ship and
switch back to the Zero G movement state.

\subsection{Weapon Firing}\label{subsec:weapons}

As mentioned, from both the player and the ship, the player is able to fire weapons.
...
pooled for performance reasons.
...
These resources get added to the player's inventory, which can be accessed by pressing \textit{I}, which
currently shows the player's current resources and the amount of each, as well as sections for the player's
...
to a planet, they will be affected by the gravity through a trigger from the planet's atmosphere, which will cause
the player to be affected by the planet's gravity.

As the gravity is custom, a custom movement system was implemented to allow for more control over the player's
movement, which involves applying a force to the player's Rigidbody component based on the player's input, as well
as a rotation force based on the surface normal of the planet, ensuring the player is always upright with respect
to the planet's surface.
Initially, this was only performed when the player ws grounded, but was changed to be applied whenever the
...
A basic life AI was implemented to simulate life on the planets, with the life wandering around the planet's
surface, and if the player is close enough following and staring at them.
...
and various transitions between the states, such as the player being close enough to the life.
The life is also damaged by the player's weapons, and can drop resources when killed.
