...
public class EnemyShipAI : MonoBehaviour
{
    ...
    [SerializeField] private SpaceMovementConfig movementConfig;
    ...
    [SerializeField] private float firingRange = 30f;
    ...
    private void FindTarget()
    {
        ...
        else
        {
            ...
            if (possibleAsteroid)
            {
                target = possibleAsteroid.transform;
            }
        }
    }
    ...
    private void ExecuteCombatBehavior()
    {
        ...
        if (!_isAiming)
        {
            float targetSpeed = Mathf.Clamp((distanceToTarget - optimalCombatRange) / 10f, -1f, 1f);
    ...
    {
        Vector3 brakeForce = -_rb.linearVelocity * brakingForce;
    ...
    private void ApplyStrafe()
    {
        ...
        {
            Vector3 randomPerp = Vector3.Cross(transform.forward, Random.insideUnitSphere).normalized;
        ...
        if (_rb.linearVelocity.magnitude < maxVelocity)
        {
            _rb.AddRelativeForce(_randomStrafeDirection * strafeForce);
        }
    }
