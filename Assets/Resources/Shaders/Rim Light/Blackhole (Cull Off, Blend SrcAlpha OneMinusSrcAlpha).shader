    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag(VertexOutput i) : COLOR
            {
                ...
                float smooth = smoothstep((hs + _HoleEdgeSmoothness), (hs - _HoleEdgeSmoothness),
                                          (1.0 - pow(1.0 - max(0, dot(normalDirection, viewDirection)), 0.15)));
                float3 cf = (smooth * tex2D(_GrabTexture,
                                            ((pow((1.0 - pow(1.0 - max(0, dot(normalDirection, viewDirection)),
                                                             _DistortionStrength)),
                                                  6.0) * (sceneUVs.rg * -2.0 + 1.0)) + sceneUVs.rg)).rgb);
                return fixed4(cf, 1);
            }
            ENDCG
        }
