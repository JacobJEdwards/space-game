...
namespace Microlight.MicroBar
{
    ...
    internal static class SimpleAnimBuilder
    {
        ...
        internal static Sequence BuildAnimation(SimpleMicroBar bar)
        {
            ...
            var anim = bar.DamageAnim;
            var isHealAnimation = bar.ParentBar.CurrentValue > bar.ParentBar.PreviousValue;
            if (isHealAnimation) anim = bar.HealAnim;
        ...
        {
            var isHealAnimation = bar.ParentBar.CurrentValue > bar.ParentBar.PreviousValue;
            var isSprite = bar.RenderType == RenderType.Sprite;
            ...
            if (bar.AdaptiveColor) sequence.OnUpdate(() => SetAdaptiveBarColor(bar));
        }
        ...
        private static void FlashAnim(SimpleMicroBar bar, Sequence sequence)
        {
            ...
            Color flashColor;
            var barColor = SetAdaptiveBarColor(bar, false);
            var ghostBarColor = SetGhostBarColor(bar);
            ...
            {
                if (bar.UseGhostBar) bar.SRGhostBar.color = flashColor;
                bar.SRPrimaryBar.color = flashColor;
            }
            else
            {
                if (bar.UseGhostBar) bar.UIGhostBar.color = flashColor;
            ...
            if (isSprite)
            {
                ...
                if (bar.UseGhostBar) sequence.Join(bar.SRGhostBar.DOColor(ghostBarColor, flashAnimDuration));
                sequence.Join(
                    srBarToAnimate.DOScaleX(bar.ParentBar.HPPercent, fillAnimDuration).SetDelay(fillAnimDelay));
            }
            else
            {
                if (isHealAnimation)
                ...
                if (bar.UseGhostBar) sequence.Join(bar.UIGhostBar.DOColor(ghostBarColor, flashAnimDuration));
                sequence.Join(uiBarToAnimate.DOFillAmount(bar.ParentBar.HPPercent, fillAnimDuration)
                    .SetDelay(fillAnimDelay));
            }
        }
        ...
        private static void SetBarValue(SimpleMicroBar bar)
        {
            if (bar.RenderType == RenderType.Image)
                ...
                bar.SRPrimaryBarMask.transform.localScale = new Vector3(bar.ParentBar.HPPercent,
                    bar.SRPrimaryBarMask.transform.localScale.y, bar.SRPrimaryBarMask.transform.localScale.z);
        }
        ...
        private static void SetGhostBarValue(SimpleMicroBar bar)
        {
            if (bar.RenderType == RenderType.Image)
                ...
                bar.SRGhostBarMask.transform.localScale = new Vector3(bar.ParentBar.HPPercent,
                    bar.SRGhostBarMask.transform.localScale.y, bar.SRGhostBarMask.transform.localScale.z);
        }

        // Sets ghost bar color if ghost bar uses dual colors
        private static Color SetGhostBarColor(SimpleMicroBar bar)
        {
            if (!bar.UseGhostBar) return Color.white;
            if (!bar.DualGhostBars)
                return bar.GhostBarDamageColor; // Also used for ghost bar color when not using dual ghost bars

            var isDamage = bar.ParentBar.CurrentValue < bar.ParentBar.PreviousValue;
        ...
        {
            if (!bar.AdaptiveColor) return bar.BarPrimaryColor;

            Color barColor;
            if (bar.RenderType == RenderType.Sprite)
            {
                if (useBarValue)
                    barColor = Color.Lerp(bar.BarAdaptiveColor, bar.BarPrimaryColor,
                        bar.SRPrimaryBarMask.transform.localScale.x);
                else
