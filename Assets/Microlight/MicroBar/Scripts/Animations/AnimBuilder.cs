...
namespace Microlight.MicroBar
{
    ...
    internal static class AnimBuilder
    {
        ...
        internal static Sequence BuildAnimation(AnimationInfo animInfo)
        {
            ...
            foreach (var x in animInfo.Commands) InterpretCommand(sequence, x, animInfo);
        ...
        /// <param name="animInfo">Data about animation</param>
        private static void InterpretCommand(Sequence sequence, AnimCommand command, AnimationInfo animInfo)
        {
            Tween tween;
        ...
        private static Tween InterpretEffect(AnimCommand command, AnimationInfo animInfo)
        {
            Tween tween = null;
            ...
            switch (command.Effect)
            {
                case AnimEffect.Color:
                    if (animInfo.IsImage)
                    {
                        newColor = InterpretValueMode(animInfo.TargetImage.color,
                            animInfo.Animation.DefaultValues.Color, command);
                        tween = animInfo.TargetImage.DOColor(newColor, command.Duration);
                    }
                    else
                    {
                        newColor = InterpretValueMode(animInfo.TargetSprite.color,
                            animInfo.Animation.DefaultValues.Color, command);
                ...
                case AnimEffect.Fill:
                    if (!animInfo.IsImage &&
                        animInfo.Animation
                            .NotBar) break; // Fill is NOT supported for the sprite bars which are not considered bars
                    ...
                    switch (command.AnimAxis)
                    {
                        case AnimAxis.Uniform:
                            if (animInfo.IsImage)
                            {
                                ...
                                tween = animInfo.TargetImage.rectTransform.DOLocalMove(
                                    new Vector3(newFloatX, newFloatY, floatZ), command.Duration);
                            }
                            else
                            {
                                ...
                                defaultFloatValue = animInfo.Animation.DefaultValues.Position.y;
                                newFloatY = InterpretValueMode(baseFloatValue, defaultFloatValue, command, false);
                                tween = animInfo.TargetSprite.transform.DOLocalMove(
                                    new Vector3(newFloatX, newFloatY, floatZ), command.Duration);
                            }
                        ...
                        case AnimAxis.XY:
                            if (animInfo.IsImage)
                            {
                                ...
                                tween = animInfo.TargetImage.rectTransform.DOLocalMove(
                                    new Vector3(newVector2.x, newVector2.y, floatZ), command.Duration);
                            }
                            else
                            {
                ...
                case AnimEffect.Rotate:
                    ...
                    if (animInfo.IsImage)
                    {
                        ...
                        newFloat = InterpretValueMode(baseFloatValue, defaultFloatValue, command, false);
                        tween = animInfo.TargetImage.rectTransform.DOLocalRotate(new Vector3(0f, 0f, newFloat),
                            command.Duration);
                    }
                    else
                    {
                        ...
                        tween = animInfo.TargetSprite.transform.DOLocalRotate(
                            new Vector3(newFloatX, newFloatY, newFloat), command.Duration);
                    }
                    ...
                    if (animInfo.IsImage)
                    {
                        ...
                        switch (command.AnimAxis)
                        {
                            case AnimAxis.Uniform:
                                ...
                                newFloatY = InterpretValueMode(baseFloatValue, defaultFloatValue, command, false);
                                tween = animInfo.TargetImage.rectTransform.DOScale(
                                    new Vector3(newFloatX, newFloatY, floatZ), command.Duration);
                                break;
                            case AnimAxis.XY:
                    ...
                    else
                    {
                        ...
                        switch (command.AnimAxis)
                        {
                            case AnimAxis.Uniform:
                                ...
                                defaultFloatValue = animInfo.Animation.DefaultValues.Scale.y;
                                newFloatY = InterpretValueMode(baseFloatValue, defaultFloatValue, command, false);
                                tween = animInfo.TargetSprite.transform.DOScale(
                                    new Vector3(newFloatX, newFloatY, floatZ), command.Duration);
                                break;
                            case AnimAxis.XY:
                ...
                case AnimEffect.Punch:
                    switch (command.TransformProperty)
                    {
                        case TransformProperties.Position:
                            if (animInfo.IsImage)
                                tween = animInfo.TargetImage.rectTransform.DOPunchPosition(command.Vector2Value,
                                    command.Duration, command.Frequency, command.PercentValue);
                            else
                                tween = animInfo.TargetSprite.transform.DOPunchPosition(command.Vector2Value,
                                    command.Duration, command.Frequency, command.PercentValue);
                            break;
                        case TransformProperties.Rotation:
                            if (animInfo.IsImage)
                                tween = animInfo.TargetImage.rectTransform.DOPunchRotation(
                                    new Vector3(0f, 0f, command.FloatValue), command.Duration, command.Frequency,
                                    command.PercentValue);
                            else
                                tween = animInfo.TargetSprite.transform.DOPunchRotation(
                                    new Vector3(0f, 0f, command.FloatValue), command.Duration, command.Frequency,
                                    command.PercentValue);
                            break;
                        case TransformProperties.Scale:
                            if (animInfo.IsImage)
                                tween = animInfo.TargetImage.rectTransform.DOPunchScale(command.Vector2Value,
                                    command.Duration, command.Frequency, command.PercentValue);
                            else
                                tween = animInfo.TargetSprite.transform.DOPunchScale(command.Vector2Value,
                                    command.Duration, command.Frequency, command.PercentValue);
                            break;
                        case TransformProperties.AnchorPosition:
                            if (!animInfo.IsImage)
                                break; // AnchorPosition is not supported for the sprites
                            tween = animInfo.TargetImage.rectTransform.DOPunchAnchorPos(command.Vector2Value,
                                command.Duration, command.Frequency, command.PercentValue);
                            break;
                        default:
                ...
                case AnimEffect.Shake:
                    switch (command.TransformProperty)
                    {
                        case TransformProperties.Position:
                            if (animInfo.IsImage)
                                tween = animInfo.TargetImage.rectTransform.DOShakePosition(command.Duration,
                                    command.FloatValue, command.Frequency);
                            else
                                tween = animInfo.TargetSprite.transform.DOShakePosition(command.Duration,
                                    command.FloatValue, command.Frequency);
                            break;
                        case TransformProperties.Rotation:
                            if (animInfo.IsImage)
                                tween = animInfo.TargetImage.rectTransform.DOShakeRotation(command.Duration,
                                    new Vector3(0f, 0f, command.FloatValue), command.Frequency);
                            else
                                tween = animInfo.TargetSprite.transform.DOShakeRotation(command.Duration,
                                    new Vector3(0f, 0f, command.FloatValue), command.Frequency);
                            break;
                        case TransformProperties.Scale:
                            if (animInfo.IsImage)
                                tween = animInfo.TargetImage.rectTransform.DOShakeScale(command.Duration,
                                    command.FloatValue, command.Frequency);
                            else
                                tween = animInfo.TargetSprite.transform.DOShakeScale(command.Duration,
                                    command.FloatValue, command.Frequency);
                            break;
                        case TransformProperties.AnchorPosition:
                            if (!animInfo.IsImage)
                                break; // AnchorPosition is not supported for the sprites
                            tween = animInfo.TargetImage.rectTransform.DOShakeAnchorPos(command.Duration,
                                command.FloatValue, command.Frequency);
                            break;
                        default:
                            Debug.LogError($"[MicroBar] Unknown 'TransformProperty': {command.TransformProperty}");
                ...
                case AnimEffect.AnchorMove:
                    ...
                    switch (command.AnimAxis)
                    {
                        case AnimAxis.Uniform:
                            ...
                            newFloatY = InterpretValueMode(baseFloatValue, defaultFloatValue, command, false);
                            tween = animInfo.TargetImage.rectTransform.DOAnchorPos(new Vector2(newFloatX, newFloatY),
                                command.Duration);
                            break;
                        case AnimAxis.XY:
            ...
            if (command.Delay > 0f) tween.SetDelay(command.Delay);
        ...
        // ****************************************************************************************************
        private static Color InterpretValueMode(Color baseValue, Color defaultValue, AnimCommand command)
        {
            if (command.ValueMode == ValueMode.Absolute) return command.ColorValue;

            if (command.ValueMode == ValueMode.Additive) return baseValue + command.ColorValue;

            if (command.ValueMode == ValueMode.Multiplicative) return baseValue * command.ColorValue;

            if (command.ValueMode == ValueMode.StartingValue) return baseValue;

            if (command.ValueMode == ValueMode.DefaultValue) return defaultValue;

            Debug.LogError($"[MicroBar] Unknown 'ValueMode': {command.ValueMode}");
            return baseValue;
        }

        private static float InterpretValueMode(float baseValue, float defaultValue, AnimCommand command, bool percent)
        {
            if (command.ValueMode == ValueMode.Absolute)
            {
                if (percent) return Mathf.Clamp01(command.PercentValue);

                return command.FloatValue;
            }

            if (command.ValueMode == ValueMode.Additive)
            {
                if (percent) return Mathf.Clamp01(baseValue + command.PercentValue);

                return baseValue + command.FloatValue;
            }

            if (command.ValueMode == ValueMode.Multiplicative)
            {
                if (percent) return Mathf.Clamp01(baseValue * command.PercentValue);
            ...
            if (command.ValueMode == ValueMode.StartingValue)
            {
                if (percent) return Mathf.Clamp01(baseValue);
        ...
        private static int InterpretValueMode(int baseValue, int defaultValue, AnimCommand command)
        {
            if (command.ValueMode == ValueMode.Absolute) return command.IntValue;

            if (command.ValueMode == ValueMode.Additive) return baseValue + command.IntValue;

            if (command.ValueMode == ValueMode.Multiplicative) return baseValue * command.IntValue;
        ...
        private static Vector3 InterpretValueMode(Vector3 baseValue, Vector3 defaultValue, AnimCommand command)
        {
            if (command.ValueMode == ValueMode.Absolute) return command.Vector3Value;

            if (command.ValueMode == ValueMode.Additive) return baseValue + command.Vector3Value;

            if (command.ValueMode == ValueMode.Multiplicative)
                return new Vector3(baseValue.x * command.Vector3Value.x, baseValue.y * command.Vector3Value.y,
                    baseValue.z * command.Vector3Value.z);
        ...
        private static Vector2 InterpretValueMode(Vector2 baseValue, Vector2 defaultValue, AnimCommand command)
        {
            if (command.ValueMode == ValueMode.Absolute) return command.Vector2Value;

            if (command.ValueMode == ValueMode.Additive) return baseValue + command.Vector2Value;
