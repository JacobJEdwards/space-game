...
namespace Microlight.MicroBar
{
    ...
    {
        [SerializeField] private SimpleMicroBar simpleBar; // Data stored about simple version of the bar
        [SerializeField] private MicroBarEditorMode editorMode; // Mode in which micro bar is
        [SerializeField] private List<MicroBarAnimation> animations; // Stores all animations for the bar
        private bool isInitalized; // Safety system that gives out warning if health bar has not been initialized
        ...
        public float PreviousValue { get; private set; } = 1f;

        private float _currentValue = 1f; // Current value of HP
        ...
        public float HPPercent => Mathf.Clamp01(CurrentValue / MaxValue);

        public static MaxHealthCalculation MaxHealthCalculation { get; private set; } =
            MaxHealthCalculation.FollowIncrease;
        ...
        public void Initialize(float maxValue)
        {
            ...
            else
            {
                ...
                if (!simpleBar.Initialize(this)) Debug.LogError("[MicroBar] 'SimpleMicroBar' initialization failed");
            }
        }
        ...
        public void SetNewMaxHP(float newMaxValue, bool skipAnimation = false)
        {
            ...
            if (newMaxValue < 1f) newMaxValue = 1f;

            var previousHealth = MaxValue;
            var previousProportion = CurrentValue / MaxValue;
            MaxValue = newMaxValue;
            var change = MaxValue - previousHealth;
            ...
            switch (MaxHealthCalculation)
            {
                ...
                case MaxHealthCalculation.FollowIncrease:
                    if (change > 0) CurrentValue += change;
                    break;
