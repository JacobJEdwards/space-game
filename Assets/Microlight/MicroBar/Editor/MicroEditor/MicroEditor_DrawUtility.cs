...
namespace Microlight.MicroEditor
{
    ...
    internal static class MicroEditor_DrawUtility
    {
        ...
        /// </summary>
        /// <returns>Returns rect with new x and y positions where should next element be</returns>
        internal static Rect DrawTwoPropertiesConstantLabel(Rect position, SerializedProperty property1,
            GUIContent label1, SerializedProperty property2, GUIContent label2, int labelWidth = 70)
        {
        ...
        /// </summary>
        /// <returns>Returns rect with new x and y positions where should next element be</returns>
        internal static Rect DrawTwoPropertiesConstantField(Rect position, SerializedProperty property1,
            GUIContent label1, SerializedProperty property2, GUIContent label2, int fieldWidth = 70)
        {
            var label1Rect = new Rect(
                position.x,
                position.y,
        ...
        {
            DrawContainer(rect, MicroEditor_Utility.ElementColor * elementColorMultiplier,
                MicroEditor_Utility.OutlineColor, MicroEditor_Utility.LightOutlineColor);
        }
        ...
        internal static void DrawContainerGlow(Rect rect, Color colorMultiplier)
        {
            ...
            EditorGUI.DrawRect(new Rect(rect.x + 2, rect.y + 1, rect.width - 4, rect.height - 2),
                MicroEditor_Utility.ElementHoverColor * colorMultiplier);
            EditorGUI.DrawRect(new Rect(rect.x + 1, rect.y + 2, 1, rect.height - 4),
                MicroEditor_Utility.ElementHoverColor * colorMultiplier);
            EditorGUI.DrawRect(new Rect(rect.xMax - 2, rect.y + 2, 1, rect.height - 4),
                MicroEditor_Utility.ElementHoverColor * colorMultiplier);
        }
        ...
        {
            EditorGUI.DrawRect(new Rect(rect.x + 1, rect.yMax - outlineWidth, rect.width - 2, outlineWidth),
                outlineColor);
        }
        ...
        internal static void DrawFadeLine(Rect rect, Color fadeColor, int lineHeight = 2, float colorBrightness = 0.3f,
            int gradientSample = 10, int sidePadding = 2)
        {
            // Initialization
            var fadeLinesWidth =
                Mathf.RoundToInt(rect.width * 0.2f /
                                 gradientSample); // 0.2 is how much (in %) of the line will fading use (per side)
            var colorGradient = colorBrightness / (gradientSample + 1);

            var leftRect = new Rect(rect.x + sidePadding - fadeLinesWidth, rect.y, rect.width, rect.height);
            var rightRect = new Rect(rect.xMax - 1 - sidePadding, rect.y, rect.width, rect.height);
            var colorMultiplier = 1f;
            for (var i = 0; i < gradientSample; i++)
            {
            ...
            // Base
            var baseRect = new Rect(leftRect.x + fadeLinesWidth, rect.y, rightRect.x - leftRect.x - fadeLinesWidth,
                lineHeight);
